...
struct fixed_point_64_32
{
    ...
    fixed_point_64_32(double d)
    {
        v = d;
        int64_t r = d * 0x100000000LL;
        f = low_word(r);
    };
...
struct fixed_point_32_32
{
    ...
    fixed_point_32_32(long long i)
    {
        v = high_word(i);
        f = low_word(i);
    };
    fixed_point_32_32(long long unsigned i)
    {
        v = high_word(i);
        f = low_word(i);
    };
    fixed_point_32_32(double d)
    {
        int64_t i = d * 0x100000000LL;
        v = high_word(i);
        f = low_word(i);
    };
    fixed_point_32_32(fixed_point_64_32 fp)
    {
        v = fp.v;
        f = fp.f;
    };
...
public:
    ...
    INLINE CoeffT filter(CoeffT value)
    {
        ...
        for (uint8_t i = 0; i < lenB; i++)
            ACCU_MUL(filtered, x[i], b_shift[i]);
        CoeffT *a_shift = &coeff_a[lenA - i_a - 1];
        for (uint8_t i = 0; i < lenA; i++)
            ACCU_MUL(filtered, y[i], a_shift[i]);
        y[i_a] = filtered;
        i_b++;
        if (i_b == lenB)
            i_b = 0;
