...
iec_w = [10, 12.5, 16, 20, 25, 31.5, 40, 50, 63, 80, ...
...
ylim([-20, 20]);
hold on;
semilogx(iec_w, ds_dB, 'g;INMP441 Datasheet plot (approx.);');
semilogx(iec_w, 20*log10(abs(ds_H)), '--c;IIR filter frequency response;');
semilogx(iec_w, (ds_dB + 20*log10(abs(eq_H))), 'b;Adjusted frequency response;', 'linewidth', 3);
legend ('boxoff');
